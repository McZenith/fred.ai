...
export const GET = async (request) => {
  try {
    ...
    // Check cache first
    const cachedData = getCachedData(seasonId);
    if (cachedData) {
    ...
    const fetchSeasonMeta = async (retries = 2) => {
      const url = `${BASE_URL}/stats_season_meta/${seasonId}${SPORTRADAR_API_KEY}`;
      ...
      for (let i = 0; i < retries; i++) {
        try {
          const response = await axios.get(url, {
            headers: {
              ...SPORTRADAR_HEADERS,
              'If-None-Match': `"${seasonId}-${timestamp}"`,
            },
          ...
          return optimizeMetadata(response.data);
        } catch (err) {
    ...
    if (process.env.NODE_ENV === 'production') {
      console.error('Season Meta API Error:', {
        seasonId: searchParams?.get('matchId'),
        error: error.message,
